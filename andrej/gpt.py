...
# mps - is referring to Metal Performance Shaders,
# which is a backend for matrix multiplication on Apple devices (macOS) with the M series of apple silicon.
# cuda - is referring to CUDA backend for matrix multiplication on Nvidia devices
device = "cpu"
mode = "raw"
mgpu = "no-gpu"
...
with open(f'trainlog/training-log-{device}-{mode}-{mgpu}.csv', mode='w', newline='') as file:
    ...
    for i in range(50):
        ...
        if first_step:
            first_step = False
            total_start_time = time.time()  # Start the timer after the first step
